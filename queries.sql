-- Queries for determining potential retirees
...
-- repeating the previous query with aliases
SELECT ri.emp_no,
    ri.first_name,
ri.last_name,
    de.to_date
...
-- Finding current employees and putting the info into a new table
SELECT ri.emp_no,
    ri.first_name,
    ri.last_name,
de.to_date
...
-- Employee count by department number with ORDER BY and modified to write to the new table "retirement_report"
SELECT COUNT(ce.emp_no), de.dept_no
INTO retirement_report
FROM current_emp as ce
LEFT JOIN dept_emp as de
ON ce.emp_no = de.emp_no
GROUP BY de.dept_no
...
-- Checking salaries table to determine data needed
SELECT * FROM salaries
...
-- joining previously created table (emp_info) with info from the salaries column
SELECT e.emp_no,
    e.first_name,
	e.last_name,
    e.gender,
...
-- return info only for the sales team
SELECT * FROM dept_info WHERE dept_name = 'Sales';

-- return only info for the sales and development teams
